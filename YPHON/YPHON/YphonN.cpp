...
double DebCut = -1.0e0;
	...
	/* handling command line options */
        for (int i=1; i<Nopt; i++) {
            ...
            } else if (!strncmp(option[i], "-RotB",4)) {
                RotB = 1; //Only restore symmetry for BEC
	...
	cellI = (CINDEX **) malloc ( (size_t) natomS*(sizeof(CINDEX *)) );
		...
		if (ii!=natomP) {
		    ...
		    if (RedDM==1 || kvec==2) atomS[n]->setxyz(cc);
		        ...
		        , atomS[n]->sym, n, c[0], c[1], c[2], atomS[n]->x, atomS[n]->y, atomS[n]->y);
              ...
              for (int x=0; x<3; x++) {
                for (int j=0; j<natomS; j++) {
                  for (int y=0; y<3; y++) {
            	    ...
            	    //if (LatDYN::Debug)
		    //	printf(" linS=%g, VlinS=%g for %d,%d\n", linS[x*3+y], VlinS[x*3+y], ii,jj);
		    //if (i!=j)Fij[iijj] -= linS[x*3+y]/(double)(natomS-1);
		    Fij[iijj] -= linS[x*3+y]/(double)(natomS);
		  }
	        } 
	      } 
	    }
	...
	/* checking the mappings between the primitive unit cell and supercell */
		...
		if (!nofetalerror) exit(1);
			...
			if (tranI) {
                            ...
                            //Fij[iijj] -= Sij[IIJJ]/(double)Kcell;
          		}
                        Fij[iijj] *= fac;
                        Fij_Orig[iijj] *= fac;
                    }
