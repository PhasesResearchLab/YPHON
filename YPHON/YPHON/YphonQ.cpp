...
class RMATRAX {
	...
	/* restore the Rotational symmetry of the force constant matirx 
	  ...
	  natomS - number of atoms in the supercell
	*/
        void ResRotSym(double *Fij_Orig, int _natomS, ATM **_atomS, double scell[3][3], int kvec, int sqs=0) {
	    natomS = _natomS;
	    atomS = _atomS;
		...
		for (int r=0; r<Nrot; r++) iiijjj[i*Nrot+r] = atomRotMap(pi, atomS[i]->sym, r, kvec, sqs);
	    }
		...
		/*
			for (int x=0; x<3; x++)
			    for (int y=0; y<3; y++) {
				fij[x][y] = Fij_Orig[(i*3+x)*natomS*3+j*3+y]/sqrt(imass*jmass);
			    }
			Add(fij, nfij, r);
			for (int x=0; x<3; x++)
			    for (int y=0; y<3; y++) nFij[(ii*3+x)*natomS*3+jj*3+y] += nfij[x][y];
		*/
		...
		for (int j=0; j<natomS; j++) {
			...
			for (int x=0; x<3; x++)
			    for (int y=0; y<3; y++) {
				Fij_Orig[(i*3+x)*natomS*3+j*3+y] =
				    nFij[(i*3+x)*natomS*3+j*3+y]*sqrt(imass*jmass)/(double)Nrot;
		        }
		}
	    }
	    */
	...
	*/
	...
	int atomRotMap(double *_xyz, char *sym, int r, int kvec=0, int sqs=0) {
		...
		int ii = 0;
		if (kvec==2) {
		    if (sqs) ii = atm.equalSQS(*atomS[i]);
		    else ii = atm.equalRot(*atomS[i]);
		} else {
		    if (sqs) ii = atm.equalSQS(*atomS[i]);
		    else ii = (atm==*atomS[i]);
		}
...
printf("why sqs compare = %d, ii=%d, natomP=%d\n", sqs, ii, natomP);
	...
	if (Rfile) {
	    ...
	    rmatrix->ResRotSym(Fij_Orig, natomS, atomS, scell, kvec, sqs);
